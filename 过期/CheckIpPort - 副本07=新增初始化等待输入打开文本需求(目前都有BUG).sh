...
# 检查RouteLog.txt是否为空，等待用户输入日志内容
echo "请在RouteLog.txt中输入日志内容并保存后，按任意键继续..."
while [[ ! -s "$logfile" ]]; do
  sleep 1
done
...
if [[ -s "$temp_file_results" ]]; then
  echo "探测完毕,打开ip.txt..."
  start "$script_dir/ip.txt"  # 在Linux中使用xdg-open，如果是Windows，可以改为start
else
  echo "没有检测到公网ip，ip.txt输出失败，按任意键继续。。。"
fi
